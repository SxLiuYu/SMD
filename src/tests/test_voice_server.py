...
class TestRateLimiting:
    ...
    def test_session_rate_limit_enforced_for_chat(self, client):
        """非default会话在达到30次/分钟限制后必须返回429"""
        from voice_server import _RATE_PER_SESSION, _check_session_rate

        sid = "rate-limit-session-test"
        blocked = 0
        for i in range(_RATE_PER_SESSION + 3):
            allowed, remaining, retry_after = _check_session_rate(sid)
            if not allowed:
                blocked += 1
        assert blocked == 3, f"应拦截3次, 实际{blocked}"

    def test_session_rate_default_session_not_limited(self, client):
        """default会话不参与会话级限流"""
        from voice_server import _check_session_rate

        for _ in range(100):
            allowed, _, _ = _check_session_rate("default")
            assert allowed
        assert voice_server._check_session_rate  # 避免未使用告警
...
class TestWebSocket:
    ...
    def test_ws_text_is_sanitized_before_brain(self, client):
        ...
        called_texts = [
            call.kwargs.get("text") or call.args[0]
            for call in mock_stream.await_args_list
        ]
        assert called_texts, "未调用 _ws_stream_and_send"
        for t in called_texts:
            assert "<script>" not in t, f"未清洗的HTML标签进入大脑: {t}"
        assert "帮我查下天气" in called_texts[0], f"清洗应剥离标签但保留正文: {called_texts[0]}"

    def test_ws_text_xss_script_tag_stripped(self, client):
        """WebSocket 消息中的 XSS 标签必须被剥除"""
        from app.auth import _sanitize_text

        dirty = '<img src=x onerror=alert(1)>天气如何'
        clean = _sanitize_text(dirty, 500)
        assert "<img" not in clean
        assert "onerror" not in clean
        assert "天气如何" in clean
